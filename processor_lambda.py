...
GEMINI_VISION_MODEL_NAME = os.environ.get('GEMINI_VISION_MODEL_NAME', 'gemini-pro-vision')
GEMINI_PICKER_MODEL_NAME = os.environ.get('GEMINI_PICKER_MODEL_NAME', 'gemini-2.5-flash')
...
def get_nutrition_data(food_item, fdc_api_key):
    ...
    try:
        ...
        logger.warning(f"Gemini picker returned invalid response: '{picker_response.text.strip()}'. Defaulting to option 1.")
...
def process_meal_from_message(message_body):
    ...
    try:
        ...
        response = processed_messages_table.get_item(Key={'idempotency_key': idempotency_key})
        ...
        if item and item.get('status') == 'PROCESSING':
            logger.warning(f"Request {idempotency_key} is already processing. Assuming previous attempt failed. Retrying.")
    ...
    write_to_google_sheets(sheet_data, google_sheets_credentials, spreadsheet_id)
